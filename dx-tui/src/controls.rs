...
use dx_diff::{Changeset, DiffLine, DiffLineKind, DiffOptions, DiffSource, DiffStats};
...
pub(crate) fn file_filter_texts(file: &dx_diff::DiffFile) -> Vec<String> {
...
pub(crate) fn file_grep_text_matches(file: &dx_diff::DiffFile, matcher: &TextMatcher) -> bool {
...
pub(crate) fn hunk_grep_text_matches(hunk: &dx_diff::DiffHunk, matcher: &TextMatcher) -> bool {
...
pub(crate) fn git_local_branch_candidate(repo: &Path) -> Option<String> {
    ...
    ["main", "master"].into_iter().find_map(|branch| {
        dx_git::branch_exists(repo, branch)
            .ok()
