...
fn f_key_filters_files_and_escape_clears_filter() {
    let changeset = changeset_with_files(&["src/lib.rs", "README.md", "dx-tui/src/lib.rs"]);
    let mut app = DiffApp::new(DiffOptions::default(), changeset, DiffLayoutMode::Unified);
    ...
    assert_eq!(visible_paths(&app), vec!["dx-tui/src/lib.rs"]);
    ...
    assert_eq!(
        visible_paths(&app),
        vec!["src/lib.rs", "README.md", "dx-tui/src/lib.rs"]
    );
...
fn live_diff_filter_ignores_non_state_git_paths() {
    let repo = std::env::temp_dir().join("dx-tui-live-filter-repo");
    let other = std::env::temp_dir().join("dx-tui-live-filter-other");
...
fn content_spans_fall_back_when_syntax_text_mismatches_diff_text() {
    let syntax = HighlightedLine {
        segments: vec![dx_syntax::SyntaxSegment {
            byte_start: 0,
            byte_end: 5,
            text: "wrong".to_owned(),
...
fn full_file_sources_cover_diff_modes_and_statuses() {
    ...
    let file = dx_diff::DiffFile {
        ...
        status: dx_diff::FileStatus::Renamed,
        hunks: Vec::new(),
        additions: 0,
        deletions: 0,
        is_binary: false,
    ...
    let deleted = dx_diff::DiffFile {
        new_path: None,
        status: dx_diff::FileStatus::Deleted,
        ..file.clone()
    };
...
fn branch_full_file_source_uses_merge_base_and_head_revision() {
    ...
    let file = dx_diff::DiffFile {
        old_path: Some("old.rs".to_owned()),
        new_path: Some("new.rs".to_owned()),
        status: dx_diff::FileStatus::Modified,
        hunks: Vec::new(),
        additions: 0,
        deletions: 0,
        is_binary: false,
...
fn changeset_with_hunks_at(repo: PathBuf, line_numbers: &[usize]) -> Changeset {
    let hunks = line_numbers
        .iter()
        .map(|line_number| dx_diff::DiffHunk {
...
fn changeset_with_hunk_line_counts(repo: PathBuf, hunks: &[(usize, usize)]) -> Changeset {
    let hunks = hunks
        .iter()
        .map(|(line_number, line_count)| dx_diff::DiffHunk {
...
fn changeset_with_files(paths: &[&str]) -> Changeset {
    let files = paths
        .iter()
        .enumerate()
        .map(|(index, path)| dx_diff::DiffFile {
