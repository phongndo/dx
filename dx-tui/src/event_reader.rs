...
impl TerminalEventReader {
    pub(crate) fn start(thread_name: &'static str) -> DxResult<Self> {
    ...
    pub(crate) async fn read_timeout(&mut self, timeout: Duration) -> DxResult<Option<Event>> {
        if timeout.is_zero() {
            return self.try_read();
        }

        match tokio::time::timeout(timeout, self.rx.recv()).await {
            Ok(Some(result)) => result.map(Some).map_err(DxError::Io),
            Ok(None) => Err(reader_stopped_error()),
            Err(_) => Ok(None),
        }
    }

    pub(crate) fn try_read(&mut self) -> DxResult<Option<Event>> {
        match self.rx.try_recv() {
            Ok(result) => result.map(Some).map_err(DxError::Io),
...
impl PausedTerminalEventReader<'_> {
    pub(crate) fn resume(mut self) -> DxResult<()> {
        self.reader.restart()?;
        self.resumed = true;
        Ok(())
    }
...
fn reader_stopped_error() -> DxError {
    DxError::Io(io::Error::other("terminal event reader stopped"))
}
...
mod tests {
    ...
    #[test]
    fn pause_discards_events_already_queued_for_dx() {
