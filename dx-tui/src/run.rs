...
pub fn run_diff(options: DiffOptions) -> DxResult<()> {
    run_diff_with_live_updates(options, true)
}

pub fn run_diff_with_live_updates(options: DiffOptions, live_updates: bool) -> DxResult<()> {
...
impl TerminalCleanup {
    pub(crate) fn install() -> DxResult<Self> {
        enable_raw_mode()?;
    ...
    pub(crate) fn cleanup(&mut self) -> DxResult<()> {
        if !self.active {
            return Ok(());
        }
        self.active = false;
