...
pub fn repository_root(repo: Option<&Path>) -> DxResult<PathBuf> {
    ...
    if root.is_empty() {
        return Err(DxError::Usage("git repository root was empty".to_owned()));
    }
...
pub fn remove_worktree(repo: &Path, path: &Path) -> DxResult<()> {
    remove_worktree_with_force(repo, path, false)
}

pub fn remove_worktree_with_force(repo: &Path, path: &Path, force: bool) -> DxResult<()> {
    let mut command = Command::new("git");
...
pub fn list_worktrees(repo: &Path) -> DxResult<Vec<GitWorktree>> {
    let output = worktree_list_output(repo)?;

    Ok(parse_worktree_list(&output.stdout))
}

pub fn main_worktree(repo: &Path) -> DxResult<PathBuf> {
    let output = worktree_list_output(repo)?;
    parse_main_worktree_path(&output.stdout).ok_or_else(|| empty_worktree_list_error(repo))
}

fn worktree_list_output(repo: &Path) -> DxResult<process::Output> {
...
pub fn worktree_state(path: &Path) -> DxResult<GitWorktreeState> {
    let output = Command::new("git")
        .arg("-C")
        .arg(path)
...
pub fn current_branch(repo: &Path) -> DxResult<Option<String>> {
    let output = Command::new("git")
        .arg("-C")
        .arg(repo)
...
pub fn current_head(repo: &Path) -> DxResult<String> {
    let output = Command::new("git")
        .arg("-C")
        .arg(repo)
    ...
    if head.is_empty() {
        return Err(DxError::Usage("git HEAD was empty".to_owned()));
    }

    Ok(head)
}

pub fn remote_url(repo: &Path, remote: &str) -> DxResult<String> {
    let output = Command::new("git")
        .arg("-C")
        .arg(repo)
    ...
    if url.is_empty() {
        return Err(DxError::Usage(format!("git remote {remote} URL was empty")));
    }

    Ok(url)
}

pub fn branch_exists(repo: &Path, branch: &str) -> DxResult<bool> {
    let output = Command::new("git")
        .arg("-C")
        .arg(repo)
...
pub fn delete_branch(repo: &Path, branch: &str) -> DxResult<()> {
    let output = Command::new("git")
        .arg("-C")
        .arg(repo)
        .args(["branch", "-D", "--"])
...
pub fn switch_branch(repo: &Path, branch: &str) -> DxResult<()> {
    let output = Command::new("git")
        .arg("-C")
        .arg(repo)
        .args(["switch", "--"])
...
pub fn switch_detached(repo: &Path) -> DxResult<()> {
    let output = Command::new("git")
        .arg("-C")
        .arg(repo)
        .args(["switch", "--detach"])
...
pub fn switch_detached_at(repo: &Path, rev: &str) -> DxResult<()> {
    let output = Command::new("git")
        .arg("-C")
        .arg(repo)
        .args(["switch", "--detach"])
...
pub fn diff_patch(repo: &Path) -> DxResult<Vec<u8>> {
...
pub fn apply_patch(repo: &Path, patch: &[u8]) -> DxResult<bool> {
...
pub fn apply_patch_reverse(repo: &Path, patch: &[u8]) -> DxResult<bool> {
    if patch.iter().all(|byte| byte.is_ascii_whitespace()) {
        return Ok(false);
    }
...
pub fn hash_bytes(repo: &Path, bytes: &[u8]) -> DxResult<String> {
    ...
    child
        .stdin
        .as_mut()
        .ok_or_else(|| DxError::Usage("failed to open git hash-object stdin".to_owned()))?
        .write_all(bytes)?;
...
fn apply_patch_command(repo: &Path, patch: &[u8], check: bool, reverse: bool) -> DxResult<()> {
    let mut command = Command::new("git");
    ...
    child
        .stdin
        .as_mut()
        .ok_or_else(|| DxError::Usage("failed to open git apply stdin".to_owned()))?
        .write_all(patch)?;
    let output = child.wait_with_output()?;
...
fn diff_patch_with_index(repo: &Path, index: Option<&Path>) -> DxResult<Vec<u8>> {
    let mut command = Command::new("git");
    command
        .arg("-C")
        .arg(repo)
...
pub fn git_path(repo: &Path, path: &str) -> DxResult<PathBuf> {
    let output = Command::new("git")
        .arg("-C")
        .arg(repo)
...
fn create_temp_index(index_path: &Path) -> DxResult<PathBuf> {
    for attempt in 0..16 {
...
fn empty_worktree_list_error(repo: &Path) -> DxError {
    DxError::Usage(format!(
...
fn git_error(context: &str, output: &std::process::Output) -> DxError {
    let stderr = String::from_utf8_lossy(&output.stderr);
    let detail = stderr.trim();
    if detail.is_empty() {
        DxError::Usage(context.to_owned())
    } else {
        DxError::Usage(format!("{context}: {detail}"))
    }
}
...
mod tests {
    ...
    fn temp_index_is_removed_when_initialization_fails() {
        let test_dir = env::temp_dir().join(format!(
            "dx-git-temp-index-cleanup-test-{}",
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
