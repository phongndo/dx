...
// release bundle hash is pinned by dx instead of trusted on first use.
...
impl SyntaxLanguageSet {
    ...
    pub fn load_with_mode(mode: SyntaxMode) -> DxResult<Self> {
...
impl SyntaxHighlighter {
    ...
    pub fn highlight(&mut self, language: &str, source: &str) -> DxResult<HighlightedText> {
        ...
        if !self.ensure_language_trusted(&language) {
            return Err(DxError::Usage(format!(
                "tree-sitter language '{language}' is not trusted; run `dx syntax add {language}`"
            )));
        }
        ...
        let config = self
            .configs
            .get(&language)
            .ok_or_else(|| DxError::Usage(format!("failed to cache {language} highlights")))?;
        let highlights = self
            .highlighter
            .highlight(config, source.as_bytes(), None, |_| None)
            .map_err(|error| DxError::Usage(format!("failed to highlight {language}: {error}")))?;
