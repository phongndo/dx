...
pub fn diff(input: DiffOptions) -> DxResult<String> {
    dx_diff::render(input)
}

pub fn diff_bytes(input: DiffOptions) -> DxResult<Vec<u8>> {
    dx_diff::render_bytes(input)
}

pub fn diff_to_writer(input: DiffOptions, writer: impl Write) -> DxResult<()> {
    dx_diff::render_to_writer(input, writer)
}
...
pub(crate) fn github_pull_request_from_target(
    ...
    github_pull_request_from_url(target).ok_or_else(|| {
        DxError::Usage("expected a pull request number or GitHub pull request URL".to_owned())
    })
}
...
pub(crate) fn local_github_pull_request(
    ...
    let root = dx_git::repository_root(repo)?;
    let remote_url = dx_git::remote_url(&root, "origin")?;
...
pub(crate) fn fetch_github_pull_request_diff(
    ...
    let mut child = ProcessCommand::new("curl")
        ...
        .map_err(|error| {
            if error.kind() == ErrorKind::NotFound {
                DxError::Usage("curl is required to fetch GitHub pull requests".to_owned())
            } else {
                DxError::Io(error)
            }
        })?;
    ...
    child
        .stdin
        .as_mut()
        .ok_or_else(|| DxError::Usage("failed to open curl config stdin".to_owned()))?
        .write_all(config.as_bytes())?;
    drop(child.stdin.take());

    let output = child.wait_with_output().map_err(DxError::Io)?;
...
pub(crate) fn github_curl_config(url: &str, token: Option<&str>) -> String {
    ...
    push_curl_config_value(&mut config, "header", "User-Agent: dx");
