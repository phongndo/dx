...
#[derive(Debug, Parser)]
#[command(name = "dx-bench", about = "dx local benchmark utilities")]
struct Cli {
    #[command(subcommand)]
...
fn measure_fixtures(args: MeasureArgs) -> BenchResult<()> {
    ...
    let options = dx_tui::DiffBenchmarkOptions {
        width: args.width,
...
fn measure_editor_reload(args: EditorReloadArgs) -> BenchResult<()> {
    ...
    let options = dx_diff::DiffOptions {
        repo: Some(repo),
        ..dx_diff::DiffOptions::default()
    };
    let path = match args.path {
        Some(path) => path,
        None => dx_diff::load_review_ref(&options)
            .map_err(|error| BenchError::Dx(error.to_string()))?
            .files
            .first()
    ...
    for _ in 0..args.iterations {
        ...
        let _ = dx_diff::load_review_ref(&options)
            .map_err(|error| BenchError::Dx(error.to_string()))?;
    ...
    for _ in 0..args.iterations {
        let start = Instant::now();
        let _ = dx_diff::load_review_ref_path(&options, &path)
            .map_err(|error| BenchError::Dx(error.to_string()))?;
...
fn measure_fixture_run(
    ...
    options: dx_tui::DiffBenchmarkOptions,
) -> BenchResult<MeasureRunReport> {
    let patch = scenario_dir.join(&manifest.paths.patch);
    let load_start = Instant::now();
    let changeset = dx_diff::load_review_ref(&dx_diff::DiffOptions {
        repo: None,
        source: dx_diff::DiffSource::Patch(dx_diff::PatchSource::File(patch)),
        scope: dx_diff::DiffScope::All,
    ...
    let tui = dx_tui::benchmark_diff_view(changeset, syntax_languages, options);
...
fn tui_report(report: dx_tui::DiffBenchmarkReport) -> TuiMeasureReport {
...
fn syntax_report(report: dx_tui::SyntaxBenchmarkReport) -> SyntaxMeasureReport {
    SyntaxMeasureReport {
...
fn minified_rust_source(tokens: usize, variant: SourceVariant) -> String {
    let mut text = String::from("pub static DX_BENCH_GENERATED: &[&str] = &[");
...
fn minified_source(tokens: usize, variant: SourceVariant) -> String {
    let mut text = String::from("const dxBenchBundle=[");
    for index in 0..tokens {
        if index > 0 {
            text.push(',');
        }
    ...
    text.push_str("];console.log(dxBenchBundle.length);");
    text
}
